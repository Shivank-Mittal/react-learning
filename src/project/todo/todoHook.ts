...
export default function useTodo() {
    ...
    const addTodo = (value: string) => {
        const newTodo = { id: Date.now(), value, isCompleted: false, isTodoEditable: false,};
    ...
    const toggleEditable = (id: number) => {
        setTodos((prev) => 
            prev.map((todo) => todo.id === id ? { ...todo, isTodoEditable: !todo.isTodoEditable } : todo )
        );
    };

    return { todos, addTodo, updateTodo, deleteTodo, toggleComplete, toggleEditable}
}