...
export class Database extends Client {
    ...
    async createPost ({title, slug, content, featuredImage, userId}: postInfo): Promise<Models.Document>{
     ...
     async updatePost( {slug, userId,  postInfo }: updatePostInfo & {userId: string}):  Promise<Models.Document> {
        return this.operationHandler<Models.Document>(
            async() => 
                await this.database.updateDocument(
                    ...
                    Conf.appWriteCOLLECTION_ID,
                    slug,
                    { title: postInfo.title, content: postInfo.content, featuredImage: postInfo.featuredImage, userId: userId }
                ),
